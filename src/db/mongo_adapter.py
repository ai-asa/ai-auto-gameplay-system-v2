...
class MongoAdapter:
    ...
    def load_settings(self, base_dir):
        ...
        self.user_name = config.get('MONGODB', 'user_name')
        self.password = config.get('MONGODB', 'password')
        self.app_name = config.get('MONGODB', 'app_name', fallback='game_logs')
    ...
    def fetch_logs(self):
        ...
        for log in logs:
            # JSONにダンプ
            text_log = json.dumps(log, ensure_ascii=False, indent=2, cls=DateTimeEncoder)
...
if __name__ == "__main__":
    ...
    ma = MongoAdapter(base_dir, "test_game", "test_save", "test_target")
    rs = "test_reasoning"
    op = "test_operation"
    bu = ["button1", "button3"]
    # ログを追加
